...
module "oasis" {
  ...
  nomad_join_name    = "${var.nomad_server_join}"
...
module "compute" {
  ...
  nomad_region       = "${var.nomad_region}"
  nomad_node_classes = "${var.nomad_node_classes}"
  nomad_join_name    = "${var.nomad_client_join}"
  datacenter         = "${var.datacenter}"
  ssh_keys           = "${var.ssh_keys}"
  private_key        = "${var.private_key}"
...
Consul servers:
    ${join("\n    ", formatlist("%s: %s/%s - ssh ubuntu@%s", split(",", module.oasis.consul_server_names), split(",", module.oasis.consul_server_private_ips), split(",", module.oasis.consul_server_public_ips), split(",", module.oasis.consul_server_public_ips)))}
...
Nomad servers:
    ${join("\n    ", formatlist("%s: %s/%s - ssh ubuntu@%s", split(",", module.oasis.nomad_server_names), split(",", module.oasis.nomad_server_private_ips), split(",", module.oasis.nomad_server_public_ips), split(",", module.oasis.nomad_server_public_ips)))}
